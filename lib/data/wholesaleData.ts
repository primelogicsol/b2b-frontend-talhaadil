export const wholesaleData = {
  hero: {
    headline: "Wholesale Partnerships — Scale Craft Trade with Volume & Reliability",
    subtext:
      "De Koshur Crafts Wholesale Partnerships enable distributors, chain stores, and large buyers to source authentic handmade crafts in bulk at competitive pricing — with full compliance, packaging, and supply-chain transparency.",
    ctaButtons: ["Start Wholesale Partnership", "Apply as a Vendor", "Apply as a Buyer"],
  },
  whyDropshipping: {
    heading: "Fixing Bulk Trade in the Craft Industry",
    problems: [
      "Lack of reliable bulk supply channels.",
      "No compliance verification for large orders.",
      "Inconsistent packaging & logistics at scale.",
      "No transparent buyer–vendor settlement system.",
    ],
    solutions: [
      "Verified wholesale buyers & vendors (KPI-checked).",
      "Smart contracts for bulk trade with compliance.",
      "Standardized & branded packaging for large shipments.",
      "KPI + retention unlocks progression to Export or Brand Growth.",
    ],
  },
  platformDifference: {
    heading: "How Our Wholesale Model Stands Apart",
    features: [
      {
        icon: "Target",
        title: "Exclusively Handmade",
        description: "Products sourced directly from artisans with authentic craftsmanship.",
      },
      {
        icon: "DollarSign",
        title: "Bulk Pricing Optimization",
        description: "Competitive volume-based pricing for large-scale distribution.",
      },
      {
        icon: "BarChart3",
        title: "AI Demand Forecasting",
        description: "Helps vendors plan production and buyers optimize inventory.",
      },
      {
        icon: "Shield",
        title: "Blockchain Traceability",
        description: "Ensures compliance and authenticity at every step of bulk trade.",
      },
      {
        icon: "Truck",
        title: "Integrated Logistics",
        description: "Logistics waivers of 5-15% for wholesale volume orders.",
      },
      {
        icon: "Package",
        title: "Retail-Ready Packaging",
        description: "Professional packaging solutions designed for wholesale distribution.",
      },
      {
        icon: "Globe2",
        title: "Carbon Tracking",
        description: "Sustainability tracking and carbon footprint monitoring for bulk orders.",
      },
      {
        icon: "ScrollText",
        title: "GI Integration",
        description: "Geographical Indication compliance for authentic craft verification.",
      },
      {
        icon: "Folder",
        title: "Trade Registry Access",
        description: "Complete craft repository and trade documentation system.",
      },
      {
        icon: "TrendingUp",
        title: "Scalable Pathway",
        description: "Most transparent route for large-volume Kashmiri handmade craft trade.",
      },
      {
        icon: "Users",
        title: "Distributor Network",
        description: "Access to verified chain stores and distribution partners globally.",
      },
      {
        icon: "Handshake",
        title: "Smart Contracts",
        description: "Automated bulk trade settlements with compliance verification.",
      },
    ],
    exclusivity: "The most scalable and transparent pathway for large-volume trade of Kashmiri handmade crafts.",
  },
  capabilities: {
    heading: "Everything You Need for Bulk Trade Success",
    sections: [
      {
        title: "Payment & Settlement Features",
        icon: "CreditCard",
        items: [
          "Escrow-based settlements for bulk transactions.",
          "Bulk invoicing & automated compliance handling.",
          "Volume-based payment scheduling and terms.",
          "Multi-currency support for international wholesale.",
        ],
      },
      {
        title: "Security Features",
        icon: "ShieldCheck",
        items: [
          "Blockchain-secured wholesale contracts.",
          "Vendor verification (GI, compliance certifications).",
          "Fraud monitoring & compliance reporting.",
          "End-to-end encryption for bulk trade data.",
        ],
      },
      {
        title: "Marketing Features",
        icon: "BarChart3",
        items: [
          "Global distributor catalog access.",
          "Featured partner placement in wholesale directories.",
          "Custom retail bundle support and promotion.",
          "Volume-based marketing incentives and campaigns.",
        ],
      },
      {
        title: "Wholesale Partnership Features",
        icon: "Package",
        items: [
          "Access to verified vendors for bulk orders.",
          "Competitive volume pricing negotiations.",
          "Priority order fulfillment and processing.",
          "KPI dashboards for monitoring performance metrics.",
        ],
      },
      {
        title: "Private Label Wholesale Features",
        icon: "Tag",
        items: [
          "White-label bulk products for select partners.",
          "Branded bulk packaging kits and customization.",
          "Retail-ready bundles and product configurations.",
          "Custom branding and packaging design services.",
        ],
      },
      {
        title: "Additional Customer Engagement",
        icon: "MessageSquare",
        items: [
          "Dedicated wholesale negotiation platform.",
          "Vendor–buyer collaboration tools and communication.",
          "KPI-linked loyalty rewards and incentives.",
          "Account management and dedicated support services.",
        ],
      },
    ],
  },
  eligibility: {
    heading: "Wholesale Entry Rules",
    requirements: [
      "KPI: 7+ required",
      "Retention: 3 months before progression",
      "Open to vendors with proven bulk supply capacity & buyers with chain or distributor networks.",
      "Lateral Entry: Possible via KHCRF Evaluation.",
    ],
  },
  pricing: {
    heading: "Fee Packages — Wholesale",
    freeOffer: {
      title: "Onboarding Fee",
      description: "$500 one-time fee for contracts and compliance setup.",
      note: "Required for all wholesale partnerships.",
    },
    packages: [
      {
        name: "Standard",
        price: "$125/month",
        features: ["Bulk order contracts", "basic packaging", "5% logistics waiver", "KPI monitoring dashboard"],
      },
      {
        name: "Growth",
        price: "$250/month",
        features: [
          "Distributor & chain store access",
          "branded packaging",
          "10% logistics waiver",
          "priority buyer allocation",
        ],
      },
      {
        name: "Premium",
        price: "$500/month",
        features: [
          "Strategic wholesale partnerships",
          "premium custom packaging",
          "15% logistics waiver",
          "dedicated account manager",
          "early progression to Export",
        ],
      },
    ],
    range: "Range: $125–$500",
  },
  technology: {
    heading: "Technology That Powers Our Platform",
    description:
      "De Koshur Crafts wholesale platform is built with enterprise-grade technology infrastructure, developed and maintained by Prime Logic Solutions, ensuring scalable bulk trade operations and seamless integration capabilities.",
    features: [
      {
        icon: "Cog",
        title: "Frameworks",
        description: "React.js, Next.js, Node.js for high-performance wholesale operations.",
      },
      {
        icon: "Cloud",
        title: "Cloud Infrastructure",
        description: "AWS cloud + serverless architecture for scalable bulk processing.",
      },
      {
        icon: "Link",
        title: "Blockchain Tools",
        description: "Smart contracts for wholesale trade verification and compliance.",
      },
      {
        icon: "BarChart3",
        title: "AI & Data",
        description: "Demand forecasting, order analytics, and KPI performance tracking.",
      },
      {
        icon: "Globe",
        title: "AR/VR Tech",
        description: "Virtual wholesale catalogs and product showcases.",
      },
      {
        icon: "Shield",
        title: "Security",
        description: "Enterprise-grade security with vendor verification and fraud prevention.",
      },
      {
        icon: "Package",
        title: "Automation",
        description: "Bulk order processing and automated fulfillment systems.",
      },
      {
        icon: "Smartphone",
        title: "Mobile Experience",
        description: "Mobile-optimized wholesale management and ordering platform.",
      },
    ],
    support:
      "Backed by Prime Logic Solutions, ensuring continuous upgrades, enterprise support, and seamless integration for wholesale operations.",
  },
  platformAdvantage: {
    heading: "Our Platform Advantage",
    description:
      "De Koshur Crafts wholesale platform leverages MACH architecture and enterprise-grade infrastructure specifically designed for bulk craft trade operations, ensuring scalability, reliability, and compliance for large-volume transactions.",
    sections: [
      {
        title: "MACH Architecture & Cloud-Based AI Tech",
        items: [
          "Microservices for bulk order processing.",
          "API-First approach for wholesale integrations.",
          "Cloud-Native hosting for scalable operations.",
          "Headless Commerce for flexible wholesale interfaces.",
          "Database Intelligence for demand forecasting.",
          "Geofencing for distributor territory management.",
        ],
      },
      {
        title: "Advanced API Integrations",
        items: [
          "ERP system integrations for wholesale operations.",
          "Fraud prevention via enterprise security protocols.",
          "Bulk order search and catalog management.",
          "Real-time inventory sync and tracking APIs.",
          "Wholesale order management and fulfillment.",
          "AI-powered demand forecasting and analytics.",
        ],
      },
      {
        title: "Powerful CMS (Content Management System)",
        items: [
          "Wholesale catalog management and workflow control.",
          "Centralized product repository for bulk operations.",
          "Personalized wholesale portal delivery.",
          "Bulk product metadata and compliance documentation.",
        ],
      },
      {
        title: "Security & Data Integrity",
        items: [
          "Enterprise-grade compliance standards.",
          "Advanced threat monitoring for bulk transactions.",
          "Multi-factor authentication for wholesale accounts.",
          "End-to-end encryption for sensitive trade data.",
          "Continuous security auditing and monitoring.",
        ],
      },
      {
        title: "Built-in Marketing Automation",
        items: [
          "AI-powered wholesale partner matching.",
          "Multi-channel distributor communication.",
          "Smart wholesale campaign management.",
          "Geographic and demographic targeting for bulk buyers.",
          "ROI-based wholesale performance tracking.",
        ],
      },
      {
        title: "Customer Engagement Tools",
        items: [
          "Digital wholesale catalogs and bulk product showcases.",
          "Product videos, bulk samples, and 3D configurators.",
        ],
      },
      {
        title: "Loyalty & Rewards Programs",
        items: [
          "Volume-based rewards and tiered wholesale benefits.",
          "Exclusive wholesale memberships and privileges.",
          "Referral incentives for distributor networks.",
          "Cross-partner wholesale collaboration programs.",
        ],
      },
      {
        title: "Customer Financing Solutions",
        items: [
          "Wholesale credit facilities and terms.",
          "Bulk order financing and payment plans.",
          "Trade credit and extended payment options.",
          "Volume-based financing solutions.",
        ],
      },
      {
        title: "Product Protection Plans",
        items: [
          "Bulk order insurance and damage coverage.",
          "Wholesale product replacement guarantees.",
          "Volume order protection and quality assurance.",
          "Customized coverage for high-value bulk shipments.",
        ],
      },
    ],
  },
  workflow: {
    heading: "Wholesale Pathway",
    steps: [
      "Apply",
      "KPI Evaluation (7+)",
      "Wholesale Onboarding",
      "Bulk Order Activation",
      "Progression to Export or Brand Growth",
    ],
  },
  faq: [
    {
      question: "What is the KPI requirement?",
      answer: "Minimum KPI 7+ with 3 months retention before progression to higher tiers.",
    },
    {
      question: "Can I move into Export?",
      answer: "Yes, with 6 months retention or via KHCRF fast-track evaluation.",
    },
    {
      question: "Do vendors need certifications?",
      answer:
        "GI/fair-trade certifications are recommended for Export progression but not mandatory for Wholesale entry.",
    },
    {
      question: "How does Wholesale differ from Consignment?",
      answer: "Wholesale involves upfront bulk supply at negotiated pricing, whereas Consignment is pay-after-sale.",
    },
    {
      question: "Can buyers request private-label bulk packaging?",
      answer: "Yes, available under Growth & Premium tiers with custom branding options.",
    },
  ],
  apiIntegration: {
    heading: "Already Have a Wholesale System? Plug & Play with Our APIs",
    description:
      "Our Wholesale platform allows direct integration with your existing ERP and distributor systems — whether you operate on SAP, Oracle NetSuite, or custom-built wholesale management solutions.",
    features: [
      {
        icon: "PlugZap",
        title: "ERP Connectors",
        description: "Pre-built integrations for SAP, Oracle NetSuite, and major ERP systems.",
      },
      {
        icon: "Wrench",
        title: "Custom System Support",
        description:
          "REST & GraphQL APIs for syncing bulk orders, inventory, and compliance with any wholesale system.",
      },
      {
        icon: "Package",
        title: "Automated Fulfillment",
        description: "Real-time bulk order routing and processing automation.",
      },
      {
        icon: "ShieldCheck",
        title: "Secure API Gateways",
        description: "Enterprise-grade encryption and authentication protocols.",
      },
      {
        icon: "BarChart3",
        title: "Analytics API",
        description: "Pull demand forecasting and KPI dashboards into your system.",
      },
      {
        icon: "Puzzle",
        title: "Custom Extensions",
        description: "Private-label, bulk packaging, and distributor management modules.",
      },
    ],
    support: {
      title: "Prime Logic Solutions Support Guarantee",
      description:
        "In case of technical difficulty or if you don't want to invest resources in API integration, our dedicated technology partner — Prime Logic Solutions (USA) — will provide wholesale integration, setup, and support services at no additional cost.",
      note: "Wholesale API documentation and sandbox available for partners.",
    },
  },
  finalCta: {
    heading: "Start Wholesale With De Koshur Crafts Today",
    subtext:
      "Expand your reach with verified vendors, blockchain-secured trade, and scalable wholesale contracts — the gateway to Export and Brand Growth.",
    ctaButtons: ["Start Wholesale Partnership", "Apply as a Vendor", "Apply as a Buyer"],
  },
}
