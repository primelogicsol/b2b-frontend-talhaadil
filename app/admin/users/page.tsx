...
import { Search, Filter, Eye, FileText, Mail, Phone, ShoppingCart, Store } from "lucide-react"
...
export default function UsersPage() {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesTab = activeTab === "all" || user.type === activeTab.slice(0, -1) // Remove 's' from 'buyers'/'vendors'
                ...
                {filteredUsers.map((user) => {
                  ...
                  return (
                    <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <div className="text-sm font-medium text-slate-900">{user.businessInfo.businessName}</div>
                          <div className="text-sm text-slate-500">{user.businessInfo.businessType}</div>
                          <div className="text-sm text-slate-500">
                            {user.businessInfo.city}, {user.businessInfo.country}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
