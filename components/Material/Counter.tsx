...
function CircularProgress({ number, maxNumber, title, isInView, is4K }: CircularProgressProps) {
...
function SlideCard({ slide, maxNumber, is4K }: SlideCardProps) {
...
export default function Counter({ slides }: CounterProps) {
  ...
  return (
    <div className={`w-full ${is4K ? "max-w-7xl" : "max-w-6xl"} mx-auto px-4 py-8`}>
      <div className="relative" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        >
          <ChevronLeft className={`${is4K ? "w-8 h-8" : "w-6 h-6"}`} />
        </button>
        ...
        >
          <ChevronRight className={`${is4K ? "w-8 h-8" : "w-6 h-6"}`} />
        </button>
          ...
          >
            {slides.map((slide) => (
              <div key={slide.id} className="flex-shrink-0" style={{ width: `${100 / slidesToShow}%` }}>
                <SlideCard slide={slide} maxNumber={maxNumber} is4K={is4K} />
              </div>
            ))}
          </motion.div>
        </div>
