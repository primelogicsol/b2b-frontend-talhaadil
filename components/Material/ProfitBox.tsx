...
import { DollarSign, Globe, Users, TrendingUp, Award, Target, Heart, Clock, Zap, Star } from "lucide-react"
...
const MetricCard: React.FC<MetricCardProps> = ({ icon, title, value, position, index }) => (
  ...
  >
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
    >
      <motion.div className="text-[var(--primary-color)]" whileHover={{ scale: 1.2 }} transition={{ duration: 0.2 }}>
        {icon}
      </motion.div>
    </motion.div>
    <motion.div whileHover={{ x: position === "left" ? 5 : -5 }} transition={{ duration: 0.2 }}>
      <motion.h3
      ...
      <motion.p className="text-gray-300 text-sm" whileHover={{ color: "#ffffff" }} transition={{ duration: 0.2 }}>
        {value}
      </motion.p>
    </motion.div>
  </motion.div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
          ...
          <motion.div
            className="bg-slate-800/40 backdrop-blur-sm rounded-2xl p-4 border border-slate-700/50 lg:mx-4 order-3 md:col-span-2 lg:col-span-1 lg:order-none"
