...
import { getNotification, filterNotificationsForUser, type Notification } from "@/services/notification"
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            ...
            try {
                ...
                const filteredNotifications = filterNotificationsForUser(allNotifications, currentUserId)
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-[var(--secondary-light-color)] z-50">
                    <div className="flex items-center justify-between p-4 border-b border-[var(--secondary-light-color)]">
                        <h3 className="text-lg font-semibold text-[var(--primary-color)]">Notifications</h3>
                        <button
                    ...
                    <div className="max-h-96 overflow-y-auto">
                        {loading ? (
                            <div className="p-4 text-center text-[var(--primary-hover-color)]">Loading notifications...</div>
                        ) : notifications.length === 0 ? (
                            <div className="p-4 text-center text-[var(--primary-hover-color)]">No notifications found</div>
                        ) : (
                            <div className="divide-y divide-[var(--secondary-light-color)]">
                                {notifications.map((notification) => {
                                    const fakeDate = new Date(Date.now() - Math.floor(Math.random() * 100000000)); // random past time
                                    const formattedDate = fakeDate.toLocaleString('en-US', {
                                        month: 'short',
                                        day: 'numeric',
                                        hour: '2-digit',
                                        minute: '2-digit'
                                    ...
                                    return (
                                        <div key={notification.id} className="p-4 hover:bg-[var(--secondary-light-color)] transition-colors">
                                            <div className="flex items-start justify-between">
                                                <div className="flex-1">
                                                    <p className="text-sm text-[var(--primary-color)] font-medium">{notification.message}</p>
